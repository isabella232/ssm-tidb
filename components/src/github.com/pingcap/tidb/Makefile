...
GOBUILD   := GOPATH=$(CURDIR)/_vendor:$(GOPATH) $(GO) build  #enable CGO  @PHILO
...
ifeq ($(TARGET), "")
	...
	$(GOBUILD) $(RACE_FLAG) -ldflags '$(LDFLAGS)' -o ./libtidb.so -buildmode=c-shared tidb-server/main.go  #build cmd for shared lib @PHILO
else
